...
#include "MainModel.h"
#include "csp/CSP.h"
#include "uthreading/UThreadContainer.h"
...
// protected region RTlogger support on begin
  ...
  else
  {
    if(argc == 3)
    {
      ...
      {
        LOGLOCAL(LOG_INFO, "Could not start remote logger.\n");
        exit(EXIT_FAILURE);
      }
...
// protected region RTlogger support end  
  ...
  LOGLOCAL(LOG_INFO, "Setting up a signal handler, press ctrl+c to stop Encodertester.\n");
  ...
  LOGLOCAL(LOG_INFO, "Constructing the Encodertester structure:\n");
...
// protected region manual tree modifications end  
  ...
  UThreadContainer* thread_container;
  createThreads(&modelThreads, &thread_container);

  // We need runnables to place user threads in a OS thread
  Set<Runnable*> runnables(1, thread_container);
  createThreads(&runnables, &modelThreads_out);
  // The created OSThreads will be placed in an output set for further processing.

  LOGLOCAL(LOG_INFO, "- Starting the threads (#%d)\n", modelThreads_out.size());

  startThreads(&modelThreads_out);

  LOGLOCAL(LOG_INFO, "Starting Encodertester - term thread: %p\n", term->getIThread());
  term->activate();
...
// protected region additional code end
  ...
  LOGLOCAL(LOG_INFO, "Encodertester stopped.\n");
